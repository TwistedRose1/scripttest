...
local clickMod = require(game:GetService("Players").YumutikTv.PlayerScripts.Aero.Controllers.UI.Click)
clickMod:Click()
...
function buyEgg()
    spawn(function()
        while wait() do
            ...
            remotePath.EggService.Purchase:FireServer(selectedEgg)
        end
    end)
end
...
local c = w:CreateFolder("Upgrader") -- Creates the folder(U will put here your buttons,etc)

local d = w:CreateFolder("Teleport")

local e = w:CreateFolder("Egg")

local f = w:CreateFolder("Gamepasses")

b:DestroyGui()

local selectedRebirth
b:Dropdown("Rebirth Amount",{"1","10","100","1000"},true,function(value)
...
local selectedEgg
e:Dropdown("Choose Egg",{"basic"},true,function(value)
    selectedEgg = value
    print(value)
end)

e:Toggle("Buy Egg",function(bool)
    getgenv().buyEgg = bool
    print("Auto Egg is now:", bool)
    if bool then
        buyEgg(selectedEgg)
    end
end)

f:Button("Buy Gamepasses",function()
    buyGamepass()
end)
